defmodule Minilytics.ApiController do
    ...
    def get_report(conn, params) do 
        ...
        query = QueryBuilder.build_stats_query(table, options)
    ...
    def extract_map(str) do
        IO.puts("extract_map , str = " <> str)
        str
        |> String.split(",") 
    ...
    def get_distinct_values(conn, params) do
        ...
        unique_values =
            ...
            |> Enum.map(fn dim -> 
                ...
                {dim, QueryBuilder.build_distinct_values_query(table,options)}
            end)
